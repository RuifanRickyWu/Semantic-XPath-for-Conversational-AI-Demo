"""
Cold Start Generator - create a task schema, domain schema, and initial XML tree
from a user request using a two-step process:
...
class TaskBootstrapper:
    ...
    def generate(
        ...
        user_facing = self._normalize_user_facing(plan_parsed.get("user_facing", {}), plan_parsed.get("task", {}))
        # emit_user_facing is handled by the caller to avoid duplicate prints.

        schema_parsed = {"schema": plan_parsed.get("schema", {})}

        content_prompt = self.content_prompt.replace(
            "<<USER_REQUEST>>",
        ...
        artifacts = self._build_artifacts_from_split(plan_parsed, schema_parsed, content_parsed, user_request)
        ...
        user_facing = self._normalize_user_facing(plan_parsed.get("user_facing", {}), artifacts.task)
    ...
    def _build_artifacts_from_split(
        ...
        content = self._normalize_content(content_parsed.get("content", content_parsed), task_schema, warnings, content_root_tag)
    ...
    def _normalize_task(self, task: Dict[str, Any], domain: Dict[str, Any], warnings: List[str]) -> Dict[str, Any]:
    ...
    def _normalize_task_schema(
        ...
        if not nodes_input:
            ...
            # Remove any wrapper nodes from schema input
            filtered_nodes = {}
            for k, v in nodes_input.items():
                ...
                if tag in reserved_tags:
                    continue
                filtered_nodes[k] = v
            nodes_input = filtered_nodes or {root_tag: {"type": "root", "fields": [], "children": []}}

        # Normalize node tags and build tag map
        tag_map: Dict[str, str] = {}
        for key in nodes_input.keys():
            tag_map[key] = self._to_title_case_tag(str(key))
        # Map any declared root to Root
        ...
        # Add placeholder nodes for missing children
    ...
    def _wrap_with_task_version(self, nodes: Dict[str, Any], content_root: str, description: str) -> Dict[str, Any]:
        wrapped = dict(nodes)
        if content_root in wrapped and wrapped.get(content_root, {}).get("type") == "root":
            wrapped[content_root]["type"] = "container"
        wrapped["Task"] = {
            "type": "root",
        ...
        wrapped["Version"] = {
            ...
            "index_attr": "number",
            "fields": ["description"],
            "children": [content_root],
        }
        # Ensure content root exists
        if content_root not in wrapped:
            wrapped[content_root] = {"type": "container", "fields": [], "children": []}
        return wrapped
    ...
    def _compute_paths(
        ...
        schema_name = task_schema.get("name", "task")
        task_name = task.get("name", "task")
        schema_dir = _STORAGE_DIR / "schemas" / "task"
        memory_path = _STORAGE_DIR / "memory" / "task" / f"{task_name}.xml"
        examples_path = _STORAGE_DIR / "prompts" / "query_generator" / "domains" / f"xpath_examples_{task_name}.txt"
    ...
    def _save_artifacts(self, artifacts: ColdStartArtifacts) -> None:
        ...
        # Ensure unique schema directory if name already exists with different content
        if schema_path.exists():
            # Find a new schema name
            ...
            while True:
                ...
                if not candidate.exists():
                    ...
                    new_examples = _STORAGE_DIR / "prompts" / "query_generator" / "domains" / f"xpath_examples_{candidate.name}.txt"
        ...
        # Refresh examples path in case schema name changed
        examples_path = Path(artifacts.paths["examples"])

        # Update data_files, default_data, examples_file in task schema
        data_name = Path(artifacts.paths["memory_xml"]).stem
        artifacts.task_schema["data_files"] = {data_name: self._relative_storage_path(Path(artifacts.paths["memory_xml"]))}
        artifacts.task_schema["default_data"] = data_name
        artifacts.task_schema["examples_file"] = self._relative_storage_path(Path(artifacts.paths["examples"]))

        # Write task schema
        with open(schema_path, "w", encoding="utf-8") as f:
            yaml.safe_dump(
                artifacts.task_schema,
                f,
                sort_keys=False,
        ...
        # Initialize or update global memory
    ...
    def _update_global_memory(self, artifacts: ColdStartArtifacts, global_memory_path: Path) -> None:
        ...
        # Attach task content directly under Version (drop task Root wrapper)
        try:
    ...
    def _build_examples_text(self, task_schema: Dict[str, Any]) -> str:
        ...
        lines = [
            ...
            "",
            "### Basic Queries",
            f"User: find items related to a topic",
    ...
    def _build_hierarchy(self, root_tag: str, nodes: Dict[str, Any]) -> str:
        lines: List[str] = []
    ...
    def _collect_subtree_nodes(self, nodes: Dict[str, Any], root_tag: str) -> Dict[str, Any]:
        if root_tag not in nodes:
            return {}
        visited = set()
        queue = [root_tag]
    ...
    def reset_all(self) -> Dict[str, Any]:
        """Reset global history and remove generated content."""
        removed = {
            "schemas": [],
            "memory": [],
            "examples": [],
        ...
        # Remove task-specific memory files (everything under storage/memory except global)
        memory_root = _STORAGE_DIR / "memory"
        for item in memory_root.iterdir():
            ...
            if item.is_dir():
                for file in item.rglob("*"):
                    if file.is_file():
                        removed["memory"].append(str(file))
                        file.unlink()
            elif item.is_file():
                removed["memory"].append(str(item))
                item.unlink()

        # Remove task-specific schemas (everything under storage/schemas except global)
        schemas_root = _STORAGE_DIR / "schemas"
        for item in schemas_root.iterdir():
            ...
            if item.is_dir():
                for file in item.rglob("*"):
                    if file.is_file():
                        removed["schemas"].append(str(file))
                        file.unlink()
            elif item.is_file():
                removed["schemas"].append(str(item))
                item.unlink()

        # Remove generated examples (keep existing if directory missing)
        examples_root = _STORAGE_DIR / "prompts" / "query_generator" / "domains"
        if examples_root.exists():
            for file in examples_root.glob("xpath_examples_*.txt"):
                removed["examples"].append(str(file))
                file.unlink()
