...
@dataclass
class IdEqPredicate(PredicateNode):
    """
    @field = "value" - exact match on node attribute/field (e.g. task_id, version_id).

    Used for registry: Task[@task_id="t1"], Version[@version_id="v1"].
    """
    field: str   # e.g. "task_id", "version_id"
    value: str   # exact value to match
    span: Optional[SourceSpan] = None

    def get_all_atomic_values(self) -> List[str]:
        return []  # No semantic scoring; exact match only
...
class AtomPredicate(PredicateNode):
    """
    field =~ "value" - local semantic match on a node's content.
...
class AggPredicate(PredicateNode):
    ...
    """
    agg_type: str  # "min" | "max" | "avg" (evidence-based) | "exists" | "prev" | "or" (legacy)
    ...
    def to_dict(self) -> Dict[str, Any]:
        result: Dict[str, Any] = {
            ...
            "inner_predicate": self.inner.to_dict(),
        }
        if self.agg_type in ("min", "max", "avg"):
            result["type"] = "evidence_agg"
            result["operator"] = f"agg_{self.agg_type}"
        else:
            result["operator"] = "AGG_PREV" if self.agg_type.lower() == "prev" else "AGG_EXISTS"
            result["child_predicate"] = result.pop("inner_predicate")
...
@dataclass
class AvgPredicate(PredicateNode):
    """
    avg(ψ1, ψ2, ...) - arithmetic mean of child predicate scores.
    """
    children: List[PredicateNode] = field(default_factory=list)
    span: Optional[SourceSpan] = None
    ...
    def __repr__(self) -> str:
        return f"avg({', '.join(str(c) for c in self.children)})"
