"""
Chat/Plan Router - routes user input to chitchat or new plan initialization.
"""
...
_PROMPT_PATH = _BASE_DIR / "storage" / "prompts" / "chat_plan_router.txt"


class ChatPlanRouter:
    def __init__(
        self,
        bootstrapper: TaskBootstrapper | None = None,
        pipeline: SemanticXPathPipeline | None = None,
        client=None,
    ):
    ...
    def route(self, user_input: str, emit_user_facing: bool = False) -> Dict[str, Any]:
        ...
        if intent == "PLAN":
            ...
            result = self.bootstrapper.generate(
                plan_request,
                save=True,
                activate=False,
                emit_user_facing=emit_user_facing,
            )
        ...
        response = decision.get("chitchat_response") or "Got it. How can I help you today?"
        return {
            "success": True,
            "mode": "chitchat",
            "response": response,
    ...
    def _classify(self, user_input: str) -> Dict[str, Any]:
