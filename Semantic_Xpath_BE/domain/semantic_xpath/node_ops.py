...
from domain.semantic_xpath.execution.execution_models import MatchedNode
...
class NodeUtils:
    ...
    def node_to_dict_schema_aware(self, node: ET.Element) -> Dict[str, Any]:
        ...
        Extracts: attributes, child elements (by schema fields or all children),
        and direct text (node.text for elements like <Item>text</Item>).
        """
        result = {
            "type": node.tag,
            "attributes": dict(node.attrib),
        ...
        fields_to_extract = schema_fields if schema_fields else [child.tag for child in node]
        ...
        for field_name in fields_to_extract:
            ...
            if elem is not None:
                ...
                result[field_name] = node.attrib[field_name]
    ...
    @staticmethod
    def node_to_path_segments(
        node: ET.Element,
        root: ET.Element,
        parent_map: Dict[ET.Element, ET.Element],
    ) -> List[Tuple[str, int]]:
        """Build walkable path as (tag, index) pairs. Index is 1-based among same-tag siblings."""
        ...
        while current is not None and current is not root:
            ...
            if parent is None:
                break
            same_tag = [c for c in parent if c.tag == current.tag]
            idx = same_tag.index(current) + 1  # 1-based
            path_parts.append((current.tag, idx))
            current = parent
        path_parts.reverse()
        return path_parts
    ...
    def node_to_xpath(
        ...
        """Build unique XPath for node, suitable for find() and delete ops."""
        path_parts = []
        current = node
        while current is not None and current is not root:
            parent = parent_map.get(current)
            ...
            part = current.tag
            if current.attrib:
                preds = [f"@{k}='{v}'" for k, v in current.attrib.items()]
                part += "[" + " and ".join(preds) + "]"
            if len(same_tag) > 1:
                part += f"[{idx}]"
